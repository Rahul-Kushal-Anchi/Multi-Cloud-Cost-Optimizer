...
def run_athena_query(session, workgroup: str, database: str, query: str) -> List[Dict]:
    ...
    while True:
        ...
        if state in ("FAILED","CANCELLED"): raise RuntimeError(f"Athena query {qid} {state}")
        if time.time() - t0 > ATHENA_TIMEOUT: raise TimeoutError("Athena query timed out")
        time.sleep(1.0)
...
def cur_cost_query_sql(table: str, days: int = 7) -> str:
    ...
    FROM {table}
    WHERE from_iso8601_timestamp(line_item_usage_start_date) >= date_add('day', -{days}, current_timestamp)
    GROUP BY 1
    ORDER BY cost DESC
    LIMIT 50
    """
...
def get_costs(days: int = 7, tenant: Tenant = Depends(get_current_tenant)):
    ...
    if not (tenant.aws_role_arn and tenant.external_id and tenant.athena_db and 
            tenant.athena_table and tenant.athena_results_bucket):
        raise HTTPException(
            status_code=400, 
    ...
    try:
        ...
        if state != "SUCCEEDED":
            error_msg = athena.get_query_execution(QueryExecutionId=qid).get("QueryExecution", {}).get("Status", {}).get("StateChangeReason", state)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error querying Athena: {str(e)}"
        )
