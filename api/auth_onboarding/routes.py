...
def signup(request: SignupRequest, session: Session = Depends(get_session)):
    ...
    tenant = Tenant(name=request.company, slug=slug, plan="starter", status="trialing")
    session.add(tenant)
    ...
    return {
        "access_token": create_access_token(
            user.email, user.id, user.role, tenant_id=tenant.id
        ),
        "tenant_id": tenant.id,
        "user_id": user.id,
        "role": user.role,
...
def login(request: LoginRequest, session: Session = Depends(get_session)):
    ...
    return {
        "access_token": create_access_token(
            user.email, user.id, user.role, tenant_id=tenant_id
        ),
...
def connect_aws(
    ...
    try:
        test_session = assume_vendor_role(
            request.aws_role_arn, request.external_id, target_region
        )
        ...
        audit = AuditLog(
            ...
            action_metadata=json.dumps(
                {"error": str(e), "role_arn": request.aws_role_arn}
            ),
        )
        session.add(audit)
        session.commit()
    ...
    if request.athena_results_prefix:
        tenant.athena_results_prefix = (
            request.athena_results_prefix or "athena-results/"
        )
    ...
    audit = AuditLog(
        ...
        action="connect_aws",
        action_metadata=json.dumps(
            {"role_arn": request.aws_role_arn, "status": "success"}
        ),
    )
    session.add(audit)
    session.commit()
