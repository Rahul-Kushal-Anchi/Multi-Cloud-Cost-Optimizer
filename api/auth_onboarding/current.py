...
class CurrentContext:
    ...
    def __init__(
        self, tenant_id: Optional[int], user_id: int, user_email: str, role: str
    ):
...
def get_current_ctx(
    ...
    if token_data.role == "global_owner":
        user = session.exec(
            select(User).where(
                User.email == token_data.sub, User.is_global_owner == True
            )
        ).first()
        tenant_id = None
    else:
        if token_data.tid is None:
            raise HTTPException(
                status_code=401,
                detail="Invalid token: tenant_id required for non-global users",
            )
        user = session.exec(
            select(User).where(
                User.email == token_data.sub, User.tenant_id == token_data.tid
            )
        ).first()
