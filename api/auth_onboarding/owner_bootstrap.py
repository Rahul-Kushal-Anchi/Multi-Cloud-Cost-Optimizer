...
def owner_signup(request: OwnerBootstrapRequest, session: Session = Depends(get_session)):
    ...
    # Return token
    return {
        "access_token": create_access_token(user.email, user.id, user.role, tenant_id=None),
        "user_id": user.id,
        "role": "global_owner",
        "email": user.email
    }
