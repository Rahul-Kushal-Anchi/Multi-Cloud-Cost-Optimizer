#!/usr/bin/env python3
"""
AWS Cost Optimizer - Complete Working Demo
Shows the complete working pipeline from data ingestion to ML predictions
"""
...
# Page config
st.set_page_config(
    page_title="AWS Cost Optimizer - Complete Demo",
    page_icon="🚀",
    layout="wide"
)
...
def generate_sample_cost_data():
    ...
    # Services with realistic cost patterns
    services = {
...
def main():
    st.title("🚀 AWS Cost Optimizer - Complete Working Demo")
    st.markdown("**End-to-End Data Pipeline Demonstration**")
    ...
    progress = st.progress(st.session_state['demo_step'] / len(steps))
    st.markdown(f"**Current Step: {steps[st.session_state['demo_step']]}**")
    ...
    # Demo completion
        ...
        st.markdown("---")
        st.header("🎉 Demo Complete!")
        
        st.markdown("""
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 15px; color: white; text-align: center;">
            <h2>✅ Complete Working Demo Pipeline</h2>
            <p style="font-size: 18px; margin: 20px 0;">This demonstration shows:</p>
            ...
            <p style="font-size: 20px; margin-top: 30px;"><strong>This is a production-ready demonstration that shows the complete data pipeline!</strong></p>
        </div>
        """, unsafe_allow_html=True)
        
        if st.button("🔄 Restart Demo", key="restart"):
            st.session_state['demo_step'] = 0
            st.rerun()
    ...
    with col1:
        if st.session_state['demo_step'] > 0:
            if st.button("⬅️ Previous Step", key="prev"):
                st.session_state['demo_step'] -= 1
                st.rerun()
    
    with col2:
        if st.session_state['demo_step'] < len(steps) - 1:
            if st.button("➡️ Next Step", key="next"):
                st.session_state['demo_step'] += 1
                st.rerun()
    
    with col3:
        if st.button("🔄 Reset Demo", key="reset"):
            st.session_state['demo_step'] = 0
            st.rerun()
